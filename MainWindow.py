...
from GradeEntry.EntryWindow import Ui_MainWindow
from GradeEntry.Log import LoggerSingleton, Logger
...
class Window(QMainWindow):
    ...
    def __init__(self, excel_operator):
        ...
        try:
            icon_path = os.path.join(os.getcwd(), "config", "ecjtu_logo.png")
        ...
        self.window_init()
        self.window_init_fill_excel_listWighet()
        ...
        self.window_init_prompt()

    def window_init_prompt(self):
        prompt = '该程序只能控制浏览打开的第一个窗口！\n\n 使用过程中请不要打开新窗口！\n\n请将需要操作的页面链接在第一个窗口打开！'
    ...
    def reset_window_control(self):
        ...
        self.ui.comboBox_excel_sheet.addItem('请先选择Excel文件')
        self.ui.comboBox_get_row.addItem('先选择Excel')
        self.ui.comboBox_get_col.addItem('先选择Excel')
    ...
    def set_my_excel_dir(self):
        try:
            ...
            excel_dir_path = QFileDialog.getExistingDirectory(self, '华东交通大学 - 成绩录入辅助软件 - 请选需要存放Excel的文件夹', current_path)
            if excel_dir_path is not None and "" != excel_dir_path:
                ...
                self.reset_window_control()
        except:
            LoggerSingleton.instance().error("Window->set_my_excel_dir 内部错误")

    def window_init_fill_excel_listWighet(self):
        try:
            ...
            LoggerSingleton.instance().error("Window->window_init_fill_excel_listWighet 内部错误")

    def fill_excel_listWidget(self):
        try:
            excel_list = self.excel_operate.get_all_excel_name_lists()
            if excel_list is None:
                QMessageBox.question(self, '问题', '不能读取该路径下的文件（没有改文件夹或没有权限读取该目录）', QMessageBox.Yes, QMessageBox.Yes)
                self.ui.listWidget_excel_list.clear()
    ...
    def start_entry(self):
        try:
            reply = QMessageBox.question(self, '请确定',
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                try:
                    ...
                    # QMessageBox
                    LoggerSingleton.instance().warning("Window->check_box 有不当的图像界面操作 该信息为正常现象")
            else:
                pass
        except:
    ...
    def re_open_browser(self):
        try:
            reply = QMessageBox.question(self, '重启浏览器', '请确定浏览器被误关闭后再继续', QMessageBox.Yes | QMessageBox.No,
                                         QMessageBox.No)
